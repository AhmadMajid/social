  ...
  describe "#set_display_name" do
    context "when display_name is set" do
      it "does not change the display_name" do
        user = build(:user, username: "jsmith", display_name: "John Smith")
        user.save
        expect(user.reload.display_name).to eq("John Smith")
      end
    end

    context "when display_name is not set" do
      it "humanizes the previously set username" do
        user = build(:user, username: "jsmith", display_name: nil)
        user.save
        expect(user.reload.display_name).to eq("Jsmith")
      end
    end
  end
end
